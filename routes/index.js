    ...
    cas.validate(key, function(err, status, username) {
      if (err) res.send({error: err});
        ...
        pool.getConnection(function(err, connection) {
          ...
          connection.query('SELECT subject, mess, location, DATE_FORMAT(time, \'%l:%i %p\') as time, lat, longit, food FROM data WHERE time - INTERVAL 1 DAY < NOW() - INTERVAL 4 HOUR AND time + INTERVAL 2 HOUR > NOW() - INTERVAL 4 HOUR ORDER by location', function(err, rows, fields) {
            if (err) 
